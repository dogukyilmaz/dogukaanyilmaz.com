        ...
        {apps.done.map((app: DouApp) => {
          ...
          return (
            <Fade in key={app.name}>
              <Box d="flex" flexDir="row" px="10px" py="10px" mt="4" bg={bg} color={color} rounded="md" shadow="md" w={500}>
                <Box>
                  <Image boxSize="140px" objectFit="cover" src={`/${app.name}.png`} alt={app.url} borderRadius="0.375em" />
                </Box>
                <Stack direction="row" my="auto" h="8em" px={5}>
                  <Divider orientation="vertical" borderColor={color === 'white' ? 'gray.200' : 'gray.700'} />
                </Stack>
                <Box my="auto">
                  <VStack divider={<StackDivider borderColor={color === 'white' ? 'gray.200' : 'gray.700'} />} spacing={4} align="baseline">
